...
RUN set -ex \
    ...
    && xray-plugin -version \
    \
    && curl -skSL $(curl -skSL 'https://api.github.com/repos/honwen/gost-plugin/releases/latest' | sed -n '/url.*linux-amd64/{s/.*\(https:.*tar.gz\).*/\1/p}') | tar xzv --strip-components=1 linux-amd64/gost-plugin \
    && gost-plugin -version
