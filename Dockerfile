...
RUN cd /usr/bin \
  ...
  yq -r '.[]|.assets[]|.browser_download_url' | grep 'x86_64-unknown-linux-musl$' | grep 'client') -o tuic-client \
  && curl -skSL $(curl -skSL 'https://api.github.com/repos/EAimTY/tuic/releases' | \
  yq -r '.[]|.assets[]|.browser_download_url' | grep 'x86_64-unknown-linux-musl$' | grep 'server') -o tuic-server \
  && chmod a+x tuic-* \
  && tuic-client -v \
  && tuic-server -v
