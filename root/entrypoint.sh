...
randstr=$(uuidgen | sed 's+-.*++g')
sed "s+dummy+${randstr}+g" -i /etc/smartdns.conf /etc/smartdns_cn.conf
...
echo >&2 "# DNSMASQ: configured[$(cat /etc/dnsmasq.conf | wc -l)]"
exec $(which dnsmasq) -k --port=$PORT
